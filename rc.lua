...
beautiful.init(string.format("%s/.config/awesome/%s/theme.lua", os.getenv("HOME"), 'kitts-theme'))
...
-- Auto-tile all windows except games or fullscreen apps
client.connect_signal("manage", function(c)
    ...
    -- If the app is a game or fullscreen, float it
    local class = c.class and c.class:lower() or ""
    local name = c.name and c.name:lower() or ""
    
    -- Other games and fullscreen apps
    if class:match("lutris") or class:match("heroic") or class:match("steam") or 
       class:match("pcsx2") or c.fullscreen then
